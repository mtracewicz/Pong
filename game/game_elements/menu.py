...
class Menu():
    def __init__(self, screen):
        self._selected = 0
        self._menu_items_text = ['1. Play',
                                 '2. Load model', '3. Save', '4. Exit']
    ...
    def draw_menu(self):
        ...
        for i in range(len(self._menu_items_text)):
            item = self._font.render(
                self._menu_items_text[i], 1, Color.RED if i == self._selected else Color.WHITE)
            self._screen.blit(
                item, (menu_horizontal_position, menu_start_position+i*50))

    def change_selected(self, direction):
        self._selected += direction
        self._selected %= len(self._menu_items_text)
